...
const { createWorkflow } = require('../src/governance/workflowCore.cjs');
const config = require('../src/governance/config.cjs');
...
test('deterministic assessment fails closed and keeps a human approval boundary', () => {
  ...
  const result = workflow.deterministicAssessment({ roleVersion:'role-v1', contentVersion:'content-v3', assessmentScore:92, passingScore:80, accessibilityStatus:'verified', policyVersion:'policy-v4' }, actor(config.assessmentRoles[0]));
  assert.equal(result.disposition, 'eligible_for_attestation');
  assert.equal(result.automatedDecision, false);
  assert.equal(result.requiresHumanReview, true);
  expectCode(() => workflow.deterministicAssessment({ roleVersion:'role-v1', contentVersion:'content-v3', assessmentScore:92, passingScore:80, accessibilityStatus:'verified', policyVersion:'policy-v4' }, actor('unauthorized_role')), 'FORBIDDEN');
});
...
test('dual-control, persistence, tenant-scope, and audit contracts are encoded', () => {
  ...
  const router = fs.readFileSync(path.join(__dirname, '../src/governance/routerFactory.cjs'), 'utf8');
