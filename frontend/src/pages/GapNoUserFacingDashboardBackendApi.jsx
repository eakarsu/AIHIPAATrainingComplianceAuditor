...
// Auto-generated page for: No user-facing dashboard (backend API only — 0 frontend page

export default function GapNoUserFacingDashboardBackendApi() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-user-facing-dashboard-backend-api', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '900px', margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>No user-facing dashboard (backend API only — 0 frontend page</h1>
      <p style={{ color: '#666', marginBottom: '1.5rem' }}>No user-facing dashboard (backend API only — 0 frontend pages)</p>
      <form onSubmit={handleSubmit} style={{ marginBottom: '1.5rem' }}>
        ...
        >
          {loading ? 'Processing...' : 'Run No user-facing dashboard (backend API only — 0 frontend page'}
        </button>
      </form>
